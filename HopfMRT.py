...
def reduction(D, delta, MRTFunc):
    ...
    x0 = np.random.rand(2)
    dt = 1e-2; T = 1000; nSteps = int(T/dt)
    t = np.linspace(0, T, nSteps)
    
    SDE.MRT_SPR(path, x0, t, MRTFunc, f, driftParams, g, 
             noiseParams, nbins=60, nIter=1000, nEvals = 11)
...
def phaseResponse(D, delta, MRTFunc):
...
def generateDataMRT(D, delta):
    ...
    #Load phase function
    MRTFunc = SDE.MRTFunc2D(f'./Hopf/d={delta:0.3f}_D={D:0.2f}')
    
    #Empirical reduction
    reduction(D, delta, MRTFunc)

    #Stats
    longTermStats(D, delta)
    
    #PRC
    buildPRC(D, delta, MRTFunc)
    
    #Phase response
    phaseResponse(D, delta, MRTFunc)
...
def main():
    ...
    for D in DList:
        generateDataMRT(D, delta1)
        generateDataMRT(D, delta2)
