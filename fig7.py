...
def HopfPanel(ax, delta, title, Ix = 0.01, Iy = 0.01):
    
    path = f'./Hopf/d={delta:0.3f}_D={sigma:0.2f}'
    
    with open(f'{path}/phaseResponsePsi.pck', 'rb') as file_handle:
    ...
    #Gradient averaging
    with open(f'{path}/empirical/empiricalPRC.pck', 'rb') as file_handle:
    ...
    ax.axhline(0, c='k')
    ax.plot(PsiBinsX2 , Ix*sPRX2, c ='aqua', lw = 6, label = r'$\epsilon_x\langle \partial_x \psi \rangle$')
    ...
    ax.plot(PsiBinsY2, Iy*sPRY2, c ='lime', lw = 6, label = r'$\epsilon_y\langle \partial_y \psi \rangle$')
...
def SNICPanel(ax, m, title, Ix = 0.01, Iy = 0.01):
    ...
    ax.plot(PsiBinsY2, Iy*sPRY2, c ='lime', lw = 6, label = r'$\epsilon_y\langle \partial_y \psi \rangle$')
    #ax.plot(PsiBinsY[1:], sPRY[1:], 'o', c ='darkgreen', label = 'Numerics')
    ax.errorbar(PsiBinsY[1:], sPRY[1:], yerr=stdErrY[1:], fmt='.', c ='darkgreen', capsize=3, label='Numerics')
...
def main():
    ...
    fig.legend(handles, labels, loc='upper center', ncol=2, fontsize=16.5)
